...
class TaxEntity:
    ...
    has_multiple_employments: bool = False
...
class TaxReasoningEngine:
    ...
    def setup_reasoner(self):
        ...
        try:
            ...
            for entity_uri, _, _ in self.graph.triples((None, RDF.type, TAX.AustrianResident)):
                ...
                # Get other income
                other_income = 0.0
                for _, _, income in self.graph.triples((entity_uri, TAX.hasOtherIncome, None)):
                    other_income = float(income)
                    break
                
                # MandatoryL1Filer classification
                if annual_wage > 14517.0:
                    has_mandatory_condition = False
                    
                    # Check for any of the mandatory conditions
                    if (entity_uri, TAX.hasIncorrectTaxCredits, Literal(True)) in self.graph:
                        has_mandatory_condition = True
                    elif (entity_uri, TAX.hasMultipleEmployments, Literal(True)) in self.graph:
                        has_mandatory_condition = True
                    elif (entity_uri, TAX.hasIncorrectCommuterAllowance, Literal(True)) in self.graph:
                        has_mandatory_condition = True
                    elif (entity_uri, TAX.hasIncorrectFamilyBonus, Literal(True)) in self.graph:
                        has_mandatory_condition = True
                ...
                # AdditionalMandatoryL1Filer classification
                if ((entity_uri, TAX.hasNoWageTax, Literal(True)) in self.graph and annual_wage >= 13308.0) or \
                   (entity_uri, TAX.hasSpecialPaymentSituations, Literal(True)) in self.graph or \
                   (entity_uri, TAX.hasDiscretionaryAssessment, Literal(True)) in self.graph:
                    self.graph.add((entity_uri, RDF.type, TAX.AdditionalMandatoryL1Filer))
                    self.graph.add((entity_uri, RDF.type, TAX.MandatoryFilingL1))
                
                # VoluntaryL1Filer classification
                has_single_employer = (entity_uri, TAX.hasSingleEmployer, Literal(True)) in self.graph
                has_correct_tax = (entity_uri, TAX.hasCorrectWageTax, Literal(True)) in self.graph
                
                # Only apply voluntary filing if not already mandatory and income is above threshold
                if not any([
                    (entity_uri, RDF.type, TAX.MandatoryL1Filer) in self.graph,
                    (entity_uri, RDF.type, TAX.AdditionalMandatoryL1Filer) in self.graph,
                    (entity_uri, RDF.type, TAX.MandatoryE1Filer) in self.graph
                ]):
                    if annual_wage > 14517.0 and has_single_employer and has_correct_tax:
                        self.graph.add((entity_uri, RDF.type, TAX.VoluntaryL1Filer))
                        self.graph.add((entity_uri, RDF.type, TAX.VoluntaryFilingL1))
                    elif annual_wage > 14517.0 and any([
                ...
                # MandatoryE1Filer classification
                if other_income > 730.0:
                    self.graph.add((entity_uri, RDF.type, TAX.MandatoryE1Filer))
                    self.graph.add((entity_uri, RDF.type, TAX.MandatoryFilingE1))
                
                # NoFilingRequired classification - when no other filing requirements apply
                if not any([
                    (entity_uri, RDF.type, TAX.MandatoryL1Filer) in self.graph,
                    (entity_uri, RDF.type, TAX.AdditionalMandatoryL1Filer) in self.graph,
    ...
    def add_entity_to_kb(self, entity: TaxEntity):
        ...
        if entity.other_income is not None:
            self.graph.add((entity_uri, TAX.hasOtherIncome,
                           Literal(entity.other_income, datatype=XSD.decimal)))
        ...
        self.graph.add((entity_uri, TAX.hasMultipleEmployments,
                       Literal(entity.has_multiple_employments, datatype=XSD.boolean)))
        ...
        self.graph.add((entity_uri, TAX.hasNoWageTax,
                       Literal(entity.has_no_wage_tax, datatype=XSD.boolean)))
        ...
        # Add E1 Filing Conditions
        self.graph.add((entity_uri, TAX.hasSelfEmploymentIncome,
                       Literal(entity.has_self_employment_income, datatype=XSD.boolean)))
